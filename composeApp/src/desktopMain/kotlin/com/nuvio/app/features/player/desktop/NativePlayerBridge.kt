...
internal object NativePlayerBridge {
    ...
    val isAvailable: Boolean by lazy {
        try {
            loadNativeLibrary()
            println("Native player bridge successfully loaded: isAvailable=true")
            true
        } catch (t: Throwable) {
            println("Native player bridge not available: ${t.message}")
            false
        }
    }
    ...
    private fun findLocalBuildLibrary(platformDir: String, libraryName: String): File? {
        ...
        val roots = listOf(
            ...
            File("composeApp/src/desktopMain/native/$platformDir/runtime"),
            File("src/desktopMain/native/$platformDir/runtime"),
        )
