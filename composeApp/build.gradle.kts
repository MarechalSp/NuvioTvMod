    ...
    fun generate() {
        ...
        outDir.resolve("com/nuvio/app/features/settings").apply {
            ...
            resolve("CommunityConfig.kt").writeText(
                ...
                |    const val CONTRIBUTIONS_URL = "${props.getProperty("CONTRIBUTIONS_URL", "https://gitserver.tapframe.space/api/unique-contributions")}" 
                |    const val SUPPORTERS_WALL_URL = "${props.getProperty("SUPPORTERS_WALL_URL", "https://nuvio.tv/api/supporters/wall")}"
                |    const val DONATIONS_BASE_URL = "${props.getProperty("DONATIONS_BASE_URL", "https://donate.nuvio.tv")}" 
                |    const val DONATIONS_DONATE_URL = "${props.getProperty("DONATIONS_DONATE_URL", "https://donate.nuvio.tv")}" 
                |}
                """.trimMargin()
            )
        }
...
val windowsMsiUpgradeUuid = "e82b7145-1c39-498b-9d41-5df408bc81a2"
val desktopPackageName = "NuvioTvMod"
val iosDistribution = (
...
if (isWindowsHost) {
    val bridgeOutput = windowsPlayerBridgeOutput.get().asFile
    bridgeOutput.parentFile.mkdirs()
    val prebuiltPlayerBridge = layout.projectDirectory.file("src/desktopMain/native/windows/runtime/player_bridge.dll").asFile
    if (prebuiltPlayerBridge.exists() && !bridgeOutput.exists()) {
        prebuiltPlayerBridge.copyTo(bridgeOutput, overwrite = true)
    }
}
...
fun publishWindowsMsiOutput(release: Boolean) {
    ...
    val finalMsi = outputDir.resolve("NuvioTvMod-Windows-$windowsPlayerBridgeArch-$desktopReleaseVersionName.msi")
    ...
    val sourceMsi = defaultMsi.takeIf { it.exists() }
        ?: outputDir.listFiles()?.firstOrNull { it.isFile && it.extension.equals("msi", ignoreCase = true) && it != finalMsi }
